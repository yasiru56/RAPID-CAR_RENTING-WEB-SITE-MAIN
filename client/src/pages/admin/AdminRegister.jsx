...
function AdminRegister({ isFirstAdmin: isFirstAdminProp }) {
  ...
  const [isFirstAdmin, setIsFirstAdmin] = useState(isFirstAdminProp || false);
  ...
  useEffect(() => {
    if (isFirstAdminProp !== undefined) {
      setIsFirstAdmin(isFirstAdminProp);
      return;
    }
    ...
    const checkExistingAdmins = async () => {
      try {
        ...
        setIsFirstAdmin(!response.data.hasAdmins);
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const endpoint = isFirstAdmin 
        ? 'http://localhost:5001/api/auth/admin/first-register' 
        : 'http://localhost:5001/api/auth/admin/register';

      const config = isFirstAdmin 
        ? {} 
        : {
            headers: {
  ...
  return (
    <div className='container-fluid min-vh-100'>
      <h1 className='display-3 mb-4 bold font-weight-bold text-center text-success'>
        {isFirstAdmin ? 'Create First Admin Account' : 'Admin Registration'}
      </h1>
      <div className='row'>
        ...
        <div className='col-md-7'>
          <div className='card p-4 shadow-lg w-100 mb-4'>
            <h3 className='text-center mb-3'>Create an Admin Account</h3>
            {error && <div className='alert alert-danger'>{error}</div>}
            {!isFirstAdmin && (
              <div className='alert alert-info'>
                You must be logged in as an admin to register new admin accounts.
              </div>
            )}
